...
class AlphaBetaAgent:
    ...
    def search(self, board, alpha, beta, maximise):
        ...
        if maximise:
            ...
            for c in children:
                ...
                new_board[c[0], c[1]] = 1
                a, _ = self.search(new_board, alpha, beta, False)
                if a >= value:
                    value = a
                    move = c
            ...
            for c in children:
                new_board = board.copy()
                new_board[c[0], c[1]] = -1
                a, _ = self.search(new_board, alpha, beta, True)
                if a <= value:
                    value = a
                    move = c
                beta = min(beta, value)
    ...
    def next_action(self):
        move = self.search(self.board.copy(), -np.inf, np.inf, True)[1]
        if move is None:
            print(None)
            return
        r, c = move
...
async def handler(websocket, path):
    ...
    async for msg in websocket:
        ...
        if msg["type"] == "start":
            if init:
                agent = AlphaBetaAgent(msg["player"])
                init = False
            else:
